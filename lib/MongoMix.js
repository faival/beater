...
var MongoMix = function(dbName) {
	this.mongoMix = mongoMix;
	if (!this.mongoMix) {
		this.dbName = dbName;
		initMongoMix();
	}
	return this.mongoMix;
	console.log('new instance of mongoMix!');
	
	console.log("return mongoMix");
};
	...
	//db.collection('songsDB', function(err, collection){
		...
		songsDB.find().toArray(function(err, songsInDb) {
			...
			var partySongsRequestComplete = function(currentSongIndex, song, beatsForSong, participantData) {
				...
				participantData[song.songId]['songData'].songUrl = "https://www.archive.org/download/"+song.songId;
